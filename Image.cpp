...
void CGLImage::CreateFromStageFile(FILE *fp)
{
	m_PixelFormat = GL_RGB;
	int iImageSize;
	long color;
	...
	if (m_PixelFormat == GL_RGB)
	{
		iImageSize = m_iWidth * m_iHeight * sizeof(CGLRGBTRIPLE);
	}
	else if (m_PixelFormat == GL_RGBA)
	{
		iImageSize = m_iWidth * m_iHeight * sizeof(CGLRGBQUAD);
	}
	m_pBits = malloc(iImageSize);
	CGLRGBTRIPLE* pRGB = (CGLRGBTRIPLE *) m_pBits;
	CGLRGBQUAD* pRGBA = (CGLRGBQUAD *) m_pBits;
	...
	for (j = 0 ; j < m_iWidth * m_iHeight; j++)
	{
		...
		if (m_PixelFormat == GL_RGB)
		{
			sscanf(&s[(j % 8) * 7], "%x", &color);
			pRGB->rgbRed = color>>16 & 0xff;
			pRGB->rgbGreen = color>>8 & 0xff;
			pRGB->rgbBlue = color & 0xff;
			pRGB++;
		}
		else if (m_PixelFormat == GL_RGBA)
		{
			sscanf(&s[(j % 8) * 9], "%x", &color);
			if (j % 128 == 0)
			{
				TRACE2("%d: color = %x\n", j, color);
			}
			pRGBA->rgbRed = color>>24 & 0xff;
			pRGBA->rgbGreen = color>>16 & 0xff;
			pRGBA->rgbBlue = color>>8 & 0xff;
			pRGBA->rgbAlpha = color & 0xff;
			pRGBA++;
		}
	}
}
...
void CGLImage::CreateFromStageFile(CArchive &ar)
{
	m_PixelFormat = GL_RGB;
	int iImageSize;
	//m_pBits = malloc(iImageSize);
	...
	for (j = 0; j < m_iWidth * m_iHeight; j++)
	{
		...
		{
			// Read in single line from file
			if (!ar.ReadString(s, 128))
			{
				AfxMessageBox("Can't read texture info");
			}
		}
		if (m_PixelFormat == GL_RGB)
		{
...
//
BOOL CGLImage::Load(LPCTSTR filename)
{
	CleanUp() ;
...
//
BOOL CGLImage::Load(WORD wResId)
{
	CleanUp() ;
...
//
void CGLImage::Translate(CDIB& aDib)
{
...
void CGLImage::TranslateByBoth(CDIB& aDib, CDIBPal& aPalSrc)
{
	...
	for (int i = 0 ; i < m_iWidth * m_iHeight ; i++)
	{
		...
		if (i % 128 == 0)
		{
			TRACE3("%02x%02x%02x  ", pDest->rgbRed, pDest->rgbGreen, pDest->rgbBlue);
		}
		pDest++;
		pSrc++ ;
	}
